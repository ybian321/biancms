...
export default function BarChart({ data }: BarChartProps) {
   ...
   useEffect(() => {
      ...
      const xCategories: string[] = uniq([...interest.map(({ name }) => name), ...Object.keys(teacher)]);
      ...
      const teacherBar: ISeriesItem[] = levels.map((level) => ({
         name: SkillDes[level],
         data: xCategories.map((lan) => teacher[lan]?.find((item: Statistic) => item.level === level)?.amount || 0),
         stack: 'teacher'
      }));
